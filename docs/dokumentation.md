...
- **API-Fehlerbehandlung bei Logout:** Beim Abmelden von geschützten Seiten (Messages, Riders, Map, Profile) wurden API-Aufrufe noch ausgelöst, obwohl die Session bereits ungültig war. Die APIs gaben dann ein Error-Objekt statt eines Arrays zurück, was zu `TypeError: conversations.map is not a function` führte. Die Lösung bestand aus mehreren Massnahmen: API-Aufrufe erst nach bestätigter Authentifizierung starten (`status === "authenticated"`), fehlende `status`-Dependencies in useEffect-Arrays ergänzen, alle Responses mit `Array.isArray()` absichern, und eine serverseitige Middleware für Route-Protection hinzufügen. Zusätzlich wurde ein Leaflet-Map-Cleanup beim Unmount ergänzt, um Memory Leaks zu verhindern.
